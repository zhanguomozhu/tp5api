...
class Editor extends TagLib
{
    ...
    public function tagUeditor($tag, $content)
    {
        ...
        $height = isset($tag['height']) ? $tag['height'] : 350;
        $length = isset($tag['length']) ? $tag['length'] : 50000;

        $parseStr = "<!-- 加载编辑器的容器 --><script id='{$id}' name='{$name}' type='text/plain' style='{$style}'>{$content}</script>";
        $parseStr .= "<!-- 实例化编辑器 --><script type='text/javascript'>UE.getEditor('{$id}',{'initialFrameWidth':'{$width}','initialFrameHeight':{$height},'maximumWords':{$length}});</script>";
